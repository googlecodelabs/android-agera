...
class CalculatorOperations {
    @NonNull
    static Result<Result<String>> keepCpuBusy(Result<String> input) {
    ...
    static Result<Integer> attemptOperation(@NonNull Pair<Integer, Integer> operands,
            @NonNull Result<Integer> operation) {
        return operation.ifSucceededAttemptMerge(operands,
                (operation1, pair1) -> {
                    try {
                        return Result.present(performOperation(pair1, operation1));
